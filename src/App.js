...
class App extends Component {
  ...
  loadMap = () => {
    ...
    mapScript.src =
      "https://maps.googleapis.com/maps/api/js?key=AIzaSyBmLVsFnIrOvRZ_14KQNDpbKEHImFEZ9Uw&callback=initMap";
    mapScript.async = true;
    mapScript.onerror = () => {
      document.write(
        "Something went terribly wrong. Your map is probably in an alternative universe! (It is not your fault, try reloading)"
      );
      window.stop();
    };
  ...
  initMap = () => {
    const cuestaDelInfierno = { lat: 37.179307, lng: -3.58826 };
  ...
  fetchInfo = location => {
    this.animateOut();
    fetch(
      `https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exchars=500&exintro&explaintext&titles=${
        location.wiki
